...
function TableResultSection({ results }: { results: AnalysisResult[] }) {
  ...
  const minWidth = 208 + results.length * 160;
  ...
  const colgroup = (
    <colgroup>
      ...
      {results.map((_, i) => <col key={i} />)}
    </colgroup>
  );

  return (
    <div>
      {/* Sticky header — overflow-x:hidden hides its own scrollbar; JS syncs scrollLeft to body */}
      <div
        ref={headRef}
        className="sticky top-16 z-20 overflow-x-hidden border border-slate-200 dark:border-slate-600 rounded-t-xl bg-slate-100 dark:bg-slate-700"
      >
        <table className="w-full text-sm" style={{ tableLayout: 'fixed', minWidth: `${minWidth}px` }}>
          {colgroup}
          <thead>
            <tr>
              <th className="px-6 py-2.5 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide sticky left-0 bg-slate-100 dark:bg-slate-700 z-10">Metric</th>
              {results.map((r, i) => (
                <th key={i} className="px-4 py-2.5 text-right text-xs font-semibold text-slate-700 dark:text-slate-200">
                  {r.botName}
                </th>
              ))}
            </tr>
      ...
      {/* Scrollable body */}
      <div ref={bodyRef} className="overflow-x-auto border border-t-0 border-slate-200 dark:border-slate-600 rounded-b-xl" onScroll={onBodyScroll}>
        <table className="w-full text-sm" style={{ tableLayout: 'fixed', minWidth: `${minWidth}px` }}>
          {colgroup}
          <tbody>
            {SECTION_DEFS.map((section) => (
              <TableSectionRows key={section.title} section={section} results={results} />
            ))}
          </tbody>
        </table>
      </div>
...
function TableSectionRows({ section, results }: { section: SectionDef; results: AnalysisResult[] }) {
  const [open, setOpen] = useState(true);

  return (
    <>
      <tr
        className="bg-slate-50 dark:bg-slate-800 border-t-2 border-slate-200 dark:border-slate-600 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700/50"
        onClick={() => setOpen(!open)}
      >
        <td colSpan={results.length + 1} className="px-6 py-3 sticky left-0">
      ...
      {open && section.rows.map((row) => (
        <tr key={row.key} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700/50">
          <td className="px-6 py-2.5 text-slate-600 dark:text-slate-400 sticky left-0 bg-white dark:bg-slate-800 z-10">
          ...
          {results.map((r, i) => {
            const val = r[row.key];
            return (
              <td key={i} className="px-4 py-2.5 text-right font-medium text-slate-900 dark:text-slate-100 whitespace-nowrap">
                {row.key === 'parseStatus' ? (
